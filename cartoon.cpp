...
void carprint(int x,int y,int i){
	...
	if(((y-2)/4*41)%2==0){				//如果骑手在横轴上 
		//cleardevice(rider[i].Riderimg);
		getimage(img,"C:/Users/70658/Desktop/car/9.jpg");
		//2.打印骑手 */
		putimage((y-2)/4*41,(x-1)/2*41+17,img);
		xyprintf((y-2)/4*41,(x-1)/2*41+17,"%d",i);
	}
	else{
		getimage(img,"C:/Users/70658/Desktop/car/7.jpg");
		putimage((y-2)/4*41+17,(x-1)/2*41,img);
		xyprintf((y-2)/4*41,(x-1)/2*41+17,"%d",i);
	}
}
...
void printorder(){
	...
	PIMAGE img;
	img = newimage();
	getimage(img,"C:/Users/70658/Desktop/car/12.jpg"); 
